...
3. При создании сервиса заполните `JIRA_AUTOMATION_WEBHOOK_URL` адресом входящего веб-хука со скриншота. `JIRA_AUTOMATION_WEBHOOK_TOKEN` нужен только при включённом секретном ключе веб-хука.
...
## Альтернативное прямое подключение

Если сервер приложения имеет сетевой доступ к Jira, можно задать `JIRA_AUTH_HEADER=Bearer <PAT>`. Тогда сервер сам выполняет постраничный POST-запрос к `/rest/api/2/search` и не использует Automation. PAT хранится только в переменных окружения сервера и никогда не передаётся в браузер.
